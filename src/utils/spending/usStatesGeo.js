...
export const getStateName = (stateId) => {
	const state = features.find((feature) => feature.id === stateId);
	return state?.properties?.name || "Unknown";
};

// Helper function to get state ID from name
export const getStateId = (stateName) => {
	const state = features.find((feature) => feature.properties.name === stateName);
	return state?.id || null;
};

// Helper function to validate state ID
export const isValidStateId = (stateId) => {
	return features.some((feature) => feature.id === stateId);
};
