...
export function FormController({
	...
	return (
		<Box sx={{ mx: "auto", p: 3 }}>
			{StepperComponent ? (
				<StepperComponent steps={steps} activeStep={activeStep} onStepClick={setActiveStep} />
			) : (
			...
			<Box sx={{ display: "flex", justifyContent: "space-between", mt: 4 }}>
				...
				<Box sx={{ display: "flex", gap: 2 }}>
					{activeStep < steps.length - 1 ? (
						<Button variant='contained' onClick={handleNext} disabled={loading} endIcon={<ArrowRight />}>
							Next
						</Button>
					) : (
